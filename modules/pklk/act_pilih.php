            ...
            <div class="row clearfix">
                <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                    <div class="card">
                        ...
                        <div class="body">
                            <form class="forms-sample" method="get" role="form" action="../../modules/pklk/act_realisasi_penggunaan_1.php">
                                ...
                                <table class="table table-bordered table-striped table-hover js-basic-example dataTable">
                                    <thead>
                                        <tr>
                                            ...
                                            <th>Belanja Modal Aset Tetap Lainnya</th>
                                        </tr>
                                    </thead>
                                    <tbody>
<?php
$no=0;
$sql = mysql_query("SELECT * FROM tb_realisasi where idu='$uidi'");
while($rs=mysql_fetch_array($sql))
	{
        
        $sqlw=mysql_query("select * from user_sekolah where idu='$uidi'");
        $user_sekolah=mysql_fetch_array($sqlw);
        ...
        $r_semester=mysql_fetch_array($sqlw);
        $sqlw=mysql_query("select * from tb_dana_bos where idu='$uidi' and id_tapel='$rsw[id_tapel]'");
        $rsbos=mysql_fetch_array($sqlw);
        $sqlw=mysql_query("select * from tb_rkas where idu='$uidi' and id_tapel='$rsw[id_tapel]' and id_semester='$r_semester[id_semester]'");
...
?>
                                        <tr>
                                        <td><?php echo"$rsw[tapel]";?></td>
                                        <td><?php echo"$r_semester[semester]";?></td>
                                        <td>Rp. <?php echo number_format((double)"$rsbos[dana]", 0, ",", ".")?>
                                        </td>
                                        <td>Rp. <?php echo number_format((double)"$rs[belanja_pegawai]", 0, ",", ".")?>
                                        </td>
                                        <td>Rp. <?php echo number_format((double)"$rs[belanja_barang]", 0, ",", ".")?>
                                        </td>
                                        <td>Rp. <?php echo number_format((double)"$rs[belanja_modal_alat]", 0, ",", ".")?>
                                        </td>
                                        <td>Rp. <?php echo number_format((double)"$rs[belanja_modal_aset]", 0, ",", ".")?>
                                        </td>
                                        </tr>
<?php
}
